...
final class NotificationsViewModel {
    ...
    func load() async {
        ...
        errorMessage = nil
        do {
            notifications = try await apiClient.request(.notifications())
        } catch {
    ...
    func markRead(_ notification: AppNotification) async {
        ...
        do {
            let _: AppNotification = try await apiClient.request(.markNotificationRead(id: notification.id))
    ...
    func markAllRead() async {
        ...
        do {
            let _: MessageResponse = try await apiClient.request(.markAllNotificationsRead)
            NotificationCenter.default.post(name: .notificationsChanged, object: nil)
        } catch {
    ...
    func delete(_ notification: AppNotification) async {
        ...
        do {
            let _: MessageResponse = try await apiClient.request(.deleteNotification(id: notification.id))
            if wasUnread {
                NotificationCenter.default.post(name: .notificationsChanged, object: nil)
            }
        } catch {
